...
enclii ps --wide
enclii ps pravara-api --env production

# Logs (tail, filter, history)
enclii logs pravara-api -f                          # live tail
enclii logs pravara-api --since 1h --level error    # last hour, errors only
enclii logs pravara-api --env staging -f
...
# Rollback
enclii rollback pravara-api                         # previous release
enclii rollback pravara-api --to-revision 5
...
enclii domains list pravara-api
enclii domains add pravara-api my.example.com       # auto-provisions tunnel route + DNS
...
# Routing (ingress + TLS)
enclii junctions list pravara-api
