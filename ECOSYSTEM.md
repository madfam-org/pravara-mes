...
**Type**: service
**Status**: production
**Registry product**: Pravara MES (`pravara-mes`, live, front door mes.madfam.io).
...
**Kubernetes namespace**: `pravara-mes`
...
### Day-to-day for pravara-api

The commands below default to `pravara-api` — the primary service name for
...
# Logs
enclii logs pravara-api --env prod -f                    # live tail
enclii logs pravara-api --env prod --since 1h -n 200     # last hour
...
# Rollback
enclii rollback pravara-api --env prod                   # previous release
enclii rollback pravara-api v42 --env prod

# Releases + deployment history
enclii releases pravara-api -n 20
...
enclii domains list --service pravara-api
enclii domains add my.example.com --service pravara-api   # auto-provisions tunnel route + DNS
